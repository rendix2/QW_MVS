...
class Smarty_Template_Source {
	...
	public static function load( Smarty_Internal_Template $_template = NULL, Smarty $smarty = NULL, $template_resource = NULL ) {
		...
		if ( ( !isset( $source->exists ) || !$source->exists ) &&
			isset( $_template->smarty->default_template_handler_func )
		) {
		...
		if ( $smarty->resource_caching && !$resource->recompiled ) {
			...
			if ( !isset( $unique_resource ) ) {
				...
				$unique_resource =
					$resource->buildUniqueResourceName( $smarty, $is_relative ? $source->filepath . $name : $name );
			}
