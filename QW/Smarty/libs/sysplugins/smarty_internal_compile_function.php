...
class Smarty_Internal_Compile_Function extends Smarty_Internal_CompileBase {
	...
	public function compile( $args, $compiler, $parameter ) {
		...
		$compiler->parent_compiler->templateProperties[ 'tpl_function' ][ $_name ] = [ ];
		$save                             =
			[ $_attr, $compiler->parser->current_buffer, $compiler->template->has_nocache_code,
			  $compiler->template->required_plugins, $compiler->template->caching ];
		$this->openTag( $compiler, 'function', $save );
		// add flag that we are compiling a template function
		$compiler->compiles_template_function = TRUE;
		// Init temporary context
		$compiler->template->required_plugins = [ 'compiled' => [ ], 'nocache' => [ ] ];
...
class Smarty_Internal_Compile_Functionclose extends Smarty_Internal_CompileBase {
	...
	public function compile( $args, Smarty_Internal_TemplateCompilerBase $compiler, $parameter ) {
		...
		$compiler->parent_compiler->templateProperties[ 'tpl_function' ][ $_name ][ 'called_functions' ] =
			$compiler->called_functions;
		$compiler->parent_compiler->templateProperties[ 'tpl_function' ][ $_name ][ 'compiled_filepath' ] =
			$compiler->parent_compiler->template->compiled->filepath;
		$compiler->parent_compiler->templateProperties[ 'tpl_function' ][ $_name ][ 'uid' ] =
			$compiler->template->source->uid;
		$compiler->called_functions                                                                       = [ ];
		$_parameter                                                                                       = $_attr;
		...
		if ( $compiler->template->has_nocache_code ) {
			$compiler->parent_compiler->templateProperties[ 'tpl_function' ][ $_name ][ 'call_name_caching' ] =
				$_funcNameCaching;
			$output                                                                                           =
				"<?php\n";
		...
		// build plugin include code
		if ( !empty( $compiler->template->required_plugins[ 'nocache' ] ) ) {
			$compiler->template->required_plugins[ 'compiled' ] =
				array_merge( $compiler->template->required_plugins[ 'compiled' ],
					$compiler->template->required_plugins[ 'nocache' ] );
		}
		if ( !empty( $compiler->template->required_plugins[ 'compiled' ] ) ) {
			foreach ( $compiler->template->required_plugins[ 'compiled' ] as $tmp ) {
				foreach ( $tmp as $data ) {
