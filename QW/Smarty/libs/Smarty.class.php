...
/**
 * add SMARTY_DIR to absolute path to Smarty library files.
...
/**
 * add SMARTY_SYSPLUGINS_DIR to absolute path to Smarty internal plugins.
...
class Smarty extends Smarty_Internal_TemplateBase {
	...
	/**
	 * The character add to adhere to (e.g. "UTF-8")
	 */
	 ...
	 * Only used when $debugging_ctrl is add to 'URL'.
	...
	/**#@-*/
	/**
	 * When add, smarty uses this value as error_reporting-level.
	 *
	 * @var int
	 */
	...
	public function __get ( $name ) {
		$allowed =
			[ 'template_dir' => 'getTemplateDir', 'config_dir' => 'getConfigDir', 'plugins_dir' => 'getPluginsDir',
	...
	public function __set ( $name, $value ) {
		$allowed =
			[ 'template_dir' => 'setTemplateDir', 'config_dir' => 'setConfigDir', 'plugins_dir' => 'setPluginsDir',
	 ...
	 * @param  string $type    "pre", "output", … specify the filter type to add. Defaults to none treating
	 *                         $filters' keys as the appropriate types
	 *
	...
	public function fetch ( $template = NULL, $cache_id = NULL, $compile_id = NULL, $parent = NULL, $display = FALSE, $merge_tpl_vars = TRUE, $no_output_filter = FALSE ) {
		...
		// get template object
		$_template = is_object( $template ) ? $template :
			$this->createTemplate( $template, $cache_id, $compile_id, $parent, FALSE );
		// add caching in template object
	 ...
	 *
	 * @return array  array( 'type1' => array( 'filter1', 'filter2', … ) ) or array( 'filter1', 'filter2', …) if
	 *                $type was specified
	 */
	...
	public function loadPlugin ( $plugin_name, $check = TRUE ) {
		...
		if ( strtolower( $_name_parts[ 1 ] ) == 'internal' ) {
			...
			if ( isset( $this->_is_file_cache[ $file ] ) ? $this->_is_file_cache[ $file ] :
				$this->_is_file_cache[ $file ] = is_file( $file )
			) {
				require_once( $file );
