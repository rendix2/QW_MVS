...
function smarty_function_html_select_time( $params ) {
	...
	if ( $display_minutes ) {
		...
		if ( $minute_id !== NULL || $all_id !== NULL ) {
			$_html_minutes .= ' id="' .
				smarty_function_escape_special_chars( $minute_id !== NULL ? ( $minute_id ? $minute_id : $_name ) :
					( $all_id ? ( $all_id . $_name ) : $_name ) ) . '"';
		}
		if ( $minute_size ) {
		...
		if ( isset( $minute_empty ) || isset( $all_empty ) ) {
			$_html_minutes .= '<option value="">' . ( isset( $minute_empty ) ? $minute_empty : $all_empty ) .
	...
	if ( $display_seconds ) {
		...
		if ( $second_id !== NULL || $all_id !== NULL ) {
			$_html_seconds .= ' id="' .
				smarty_function_escape_special_chars( $second_id !== NULL ? ( $second_id ? $second_id : $_name ) :
					( $all_id ? ( $all_id . $_name ) : $_name ) ) . '"';
		}
		if ( $second_size ) {
		...
		if ( isset( $second_empty ) || isset( $all_empty ) ) {
			$_html_seconds .= '<option value="">' . ( isset( $second_empty ) ? $second_empty : $all_empty ) .
				'</option>' . $option_separator;
		}
	...
	if ( $display_meridian && !$use_24_hours ) {
		...
		if ( $meridian_id !== NULL || $all_id !== NULL ) {
			$_html_meridian .= ' id="' .
				smarty_function_escape_special_chars( $meridian_id !== NULL ? ( $meridian_id ? $meridian_id : $_name ) :
					( $all_id ? ( $all_id . $_name ) : $_name ) ) . '"';
		}
		if ( $meridian_size ) {
		...
		$_html_meridian .= '<option value="am"' . ( $_hour > 0 && $_hour < 12 ? ' selected="selected"' : '' ) .
			'>AM</option>' . $option_separator . '<option value="pm"' . ( $_hour < 12 ? '' : ' selected="selected"' ) .
			'>PM</option>' . $option_separator . '</select>';
	}
