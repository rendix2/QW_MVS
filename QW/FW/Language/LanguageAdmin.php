...
class LanguageAdmin extends Language {
	...
	public function __construct ( $langName ) {
		...
		if ( !file_exists( self::PATH . $this->langName . self::PREFIX_NAME . $this->langName .
			self::EXT )
		) throw new LanguageException( 'Neexistující jazykový balíček administrace: <strong>' . $langName .
			'</strong>.' );

		if ( !file_exists( self::PATH . $this->langName . self::PREFIX_NAME_META . $this->langName .
			self::EXT )
		) throw new LanguageException( 'Neexistující jazykový balíček meta dat administrace: <strong>' . $langName .
			'</strong>.' );

		$this->langAdmin =
			parse_ini_file( self::PATH . $this->langName . self::PREFIX_NAME . $this->langName . self::EXT );
		$this->metaAdmin =
