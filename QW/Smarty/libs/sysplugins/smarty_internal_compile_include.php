		...
		if ( $merge_compiled_includes ) {
			...
			if ( !isset( $compiler->parent_compiler->mergedSubTemplatesData[ $tpl_name ][ $uid ] ) ) {
				...
				if ( !( $tpl->source->uncompiled ) && $tpl->source->exists ) {
					...
					$compiler->parent_compiler->mergedSubTemplatesData[ $tpl_name ][ $uid ][ 'nocache_hash' ] =
					$tpl->properties[ 'nocache_hash' ] = str_replace( [ '.', ',' ], '_', uniqid( rand(), TRUE ) );
					// get compiled code
					$compiled_code = Smarty_Internal_Extension_CodeFrame::createFunctionFrame( $tpl,
						$tpl->compiler->compileTemplate( $tpl, NULL, $compiler->parent_compiler ) );
					unset( $tpl->compiler );

					// remove header code
					$compiled_code =
						preg_replace( "/(<\?php \/\*%%SmartyHeaderCode:{$tpl->properties['nocache_hash']}%%\*\/(.+?)\/\*\/%%SmartyHeaderCode%%\*\/\?>\n)/s",
							'', $compiled_code );
					if ( $tpl->has_nocache_code ) {
						// replace nocache_hash
						$compiled_code = str_replace( "{$tpl->properties['nocache_hash']}",
							$compiler->template->properties[ 'nocache_hash' ], $compiled_code );
						$compiler->template->has_nocache_code = TRUE;
					}
					$compiler->parent_compiler->mergedSubTemplatesCode[ $tpl->properties[ 'unifunc' ] ] =
						$compiled_code;
